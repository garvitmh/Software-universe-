...
import { CS_CORE } from "./content/cs-core";
import { SYSTEMS_NETWORKS } from "./content/systems-networks";
import { CRAFT_SD } from "./content/craft-systemdesign";
import { WEB_DEEP } from "./content/web-deep";
import { BACKEND_DEEP } from "./content/backend-deep";
import { AI_SEC_DEEP } from "./content/ai-security-deep";
import { MOBILE_DEVOPS_DEEP } from "./content/mobile-devops-deep";
import { CLOUD_DATAENG_DEEP } from "./content/cloud-dataeng-deep";
import { FOUNDATIONS_DEEP } from "./content/foundations-deep";
import { DATA_DEEP } from "./content/data-deep";
import { WEB_PLATFORM_DEEP } from "./content/web-platform-deep";
import { SYSTEMS_OPS_DEEP } from "./content/systems-ops-deep";
...
const BASE_CONTENT = {
  ...
  // ───────────────────────── BREADTH: WEB / API STYLES ─────────────────────────
  "api-styles": {
    slug: "api-styles",
    title: "API styles — REST, GraphQL, gRPC, WebSockets",
    category: "Backend",
    color: "amber",
    tagline: "Four ways for programs to talk — and when each one fits.",
    oneLiner: "REST, GraphQL, gRPC and WebSockets are different shapes of conversation between programs; the right one depends on who's talking and how.",
    what: [
      "Once you have a backend, the question is *how* clients talk to it — and there's more than one answer. **REST** (which you've met) organises everything around resources at URLs, using plain HTTP verbs. Simple, universal, cacheable — the default for most web APIs.",
      "**GraphQL** flips control to the client: instead of many fixed endpoints, there's one endpoint and the client asks for exactly the fields it wants in a single query. Great when different screens need different slices of data and you want to avoid over- or under-fetching.",
      "**gRPC** is built for speed between *services* (backend-to-backend): compact binary messages and a strict contract, so internal calls are fast and typed. **WebSockets** open a permanent two-way channel, so the server can push updates the instant they happen — chat, live dashboards, multiplayer.",
    ],
    analogy: {
      title: "Letters, à la carte, shorthand, and a phone line",
      body: "REST is sending letters to fixed addresses — simple and universal. GraphQL is an à-la-carte menu: ask for exactly the dishes you want, no more. gRPC is two colleagues who share a private shorthand — terse and fast. WebSockets is keeping a phone line open so either side can speak the moment there's news. You pick by who's talking, and how often.",
    },
    insideTitle: "The four, at a glance",
    inside: [
      { name: "REST", desc: "Resources at URLs + HTTP verbs. Simple, cacheable, universal — the sensible default." },
      { name: "GraphQL", desc: "One endpoint; the client asks for exactly the fields it needs in one query." },
      { name: "gRPC", desc: "Fast, typed binary calls between backend services. Not browser-native." },
      { name: "WebSockets", desc: "A persistent two-way channel for real-time push (chat, live updates)." },
    ],
    how: [
      "REST and GraphQL ride on ordinary HTTP, so any client can use them; REST spreads data across many endpoints, GraphQL puts it behind one flexible query. gRPC uses HTTP/2 and a binary format (Protocol Buffers) with a shared schema, so both sides agree on the exact message shape — fast, but it needs tooling and isn't directly usable from a browser.",
      "WebSockets start as a normal HTTP request that then ‘upgrades' to a persistent connection, after which messages flow both ways with no new request each time — which is what makes live updates feel instant.",
    ],
    why: [
      "Each exists because the others fall short somewhere. REST under-fetches (a screen needs three calls) or over-fetches (you get fields you ignore); GraphQL fixes that for rich, varied UIs. REST and GraphQL are too chatty for high-volume internal calls; gRPC is built for that. None of them push — so for anything live you reach for WebSockets (or server-sent events).",
      "Most real systems **mix** them: REST or GraphQL at the edge for clients, gRPC between internal services, WebSockets for the live bits.",
    ],
    alternatives: [
      { name: "Server-Sent Events (SSE)", note: "One-way server→client push over plain HTTP — simpler than WebSockets when you only need to receive." },
      { name: "tRPC", note: "Typed RPC for TypeScript-everywhere stacks — REST-like ergonomics with end-to-end types." },
    ],
    breaks: "Reach for GraphQL on a tiny app and you've added a query language, a schema, and caching headaches to a problem you didn't have. Expose gRPC straight to browsers and it won't work — it isn't browser-native. Hold thousands of idle WebSocket connections without planning for it and you exhaust server memory. And GraphQL's flexibility lets a client request a monstrously expensive query unless you add depth limits. Each style fits a situation, not a universal upgrade.",
    scale: "A small product is well served by REST alone. As the front-end grows varied, GraphQL tames over-fetching. As you split into many backend services, gRPC keeps internal calls fast. At large scale you run all of them deliberately — an API gateway out front, a typed binary mesh inside, and a separate real-time tier for WebSockets — each scaled on its own.",
    related: ["http-rest", "realtime-sync", "rate-limiting"],
  },

  "css-accessibility": {
    slug: "css-accessibility",
    title: "CSS, layout & accessibility",
    category: "Web & Frontend",
    color: "blue",
    tagline: "Making a page look right for everyone — including people who can't see it.",
    oneLiner: "CSS controls how a page looks and lays out; accessibility makes sure that look works for every user, including those using a screen reader or keyboard.",
    what: [
      "HTML says *what* is on a page (a heading, a button, a list); **CSS (Cascading Style Sheets)** says *how* it looks — colours, spacing, fonts, and how things arrange themselves as the screen changes size. It's a separate language from the HTML, so structure and styling stay independent.",
      "Modern layout is mostly two tools: **Flexbox** (arrange things in a row or column, distributing space) and **Grid** (two-dimensional layouts). With **media queries**, the same page rearranges itself for a phone, a tablet, or a wide monitor — that's **responsive design**.",
      "**Accessibility (a11y)** is the discipline of making sure all of that actually works for everyone — including people who navigate by keyboard, or who can't see the screen and rely on a **screen reader** that reads the page aloud. For many users it's the difference between using your app and being locked out.",
    ],
    analogy: {
      title: "A building's paint vs its ramps",
      body: "If HTML is a building's structure, CSS is the paint, lighting and furniture. Accessibility is the ramps, the braille on the lift buttons, the clear signage — the things that decide whether *everyone* can get around, not just people who arrive a particular way. A beautiful building with no ramp is still a failure of design.",
    },
    insideTitle: "The pieces",
    inside: [
      { name: "The cascade", desc: "Rules combine and override by specificity and order — the ‘C' in CSS. Powerful, occasionally maddening." },
      { name: "Flexbox & Grid", desc: "The two layout engines: one-dimensional rows/columns, and two-dimensional grids." },
      { name: "Media queries", desc: "‘When the screen is this wide, change these styles' — the basis of responsive design." },
      { name: "Semantic HTML & ARIA", desc: "Use the right element (button, nav, heading) so assistive tech understands the page; ARIA fills the gaps." },
    ],
    how: [
      "CSS rules are selectors (which elements) plus declarations (what to change). When several rules touch the same element, the **cascade** decides who wins by specificity and source order. Layout flows from Flexbox/Grid, and media queries swap rules at breakpoints so the page reflows for the device.",
      "Accessibility rides mostly on **semantic HTML**: use a real `<button>` (focusable, keyboard-activatable, announced as a button) instead of a clickable `<div>`. Headings in order give a screen-reader user a table of contents. Enough colour contrast, visible focus outlines, and `alt` text on images cover the basics; ARIA attributes patch anything HTML can't express.",
    ],
    why: [
      "Separating CSS from HTML lets you restyle a whole site without touching its content, and reuse styles everywhere. Flexbox/Grid replaced a decade of hacks because they describe layout *intent* directly. And accessibility — beyond being right — is often a legal requirement, improves SEO, and (because semantic, keyboard-friendly pages are simply better-built) tends to improve the experience for everyone.",
      "The alternative — styling with inline hacks and div-soup, accessibility bolted on later — produces brittle layouts and apps that exclude real users.",
    ],
    breaks: "Build buttons out of styled `<div>`s and a keyboard user can't reach them, a screen reader doesn't announce them, and your app is unusable for a chunk of people. Skip contrast checks and text vanishes for anyone in sunlight or with low vision. Forget `alt` text and images are silence to a screen reader. Lay out with fixed pixel widths and the page breaks on a phone. Most of this is invisible to a developer testing on their own laptop — which is exactly why it's so often shipped broken.",
    scale: "A small site needs semantic HTML, responsive layout, and contrast — cheap and high-impact. As a product grows you add a design system of shared accessible components and automated a11y tests in CI. At large scale accessibility becomes a tracked requirement with audits and real screen-reader testing, because at a million users even a 1%-excluded edge case is ten thousand people shut out.",
    related: ["rendering", "nextjs"],
  },

  // ───────────────────── BREADTH: CLOUD / AI / SECURITY (final) ─────────────────────
  "managed-services": {
    slug: "managed-services",
    title: "Managed databases & queues",
    category: "Cloud",
    color: "blue",
    tagline: "Letting the cloud run your database, cache and queue so you don't have to.",
    oneLiner: "A managed service is infrastructure (a database, queue, cache) the cloud operates for you — you use it; they handle backups, patching, scaling and failover.",
    what: [
      "Running a database yourself means installing it, configuring it, backing it up, patching security holes, monitoring it at 3am, and replacing the machine when it dies. A **managed service** hands all of that to the cloud provider: you get a database (or queue, or cache) as a ready-to-use endpoint, and they run the boring, critical operations behind it.",
      "Examples are everywhere: Amazon RDS or Cloud SQL (managed Postgres/MySQL), managed Redis, managed Kafka/SQS (queues). You connect and use them like the real thing — because they *are* the real thing, just operated for you.",
      "The trade is **control for convenience and reliability**. You give up some low-level tuning and pay a premium, and in return you stop spending your nights doing database operations.",
    ],
    analogy: {
      title: "Renting vs owning the boiler",
      body: "Owning a building means you own the boiler — and every burst pipe and midnight breakdown is yours. A managed service is a serviced office where the building handles heating, plumbing and security. You plug in and work; someone else gets the 3am call when something fails.",
    },
    insideTitle: "What's managed (and what isn't)",
    inside: [
      { name: "Managed database", desc: "Postgres/MySQL/Mongo run for you — automated backups, failover, version upgrades." },
      { name: "Managed cache", desc: "A hosted Redis/Memcached, scaled and patched by the provider." },
      { name: "Managed queue/stream", desc: "SQS, Kafka, Pub/Sub — durable messaging without running brokers yourself." },
      { name: "What you still own", desc: "Your schema, queries and data model — they run the box, not your design." },
    ],
    how: [
      "You provision the service (size, region, replicas), get a connection endpoint, and use it from your app exactly as you would a self-hosted one. The provider continuously handles backups, security patches, monitoring, and — for databases — automatic failover to a standby if the primary dies.",
      "Scaling is often a setting: bump the instance size, add read replicas, or enable autoscaling, instead of a migration project. You pay for what you provision (and sometimes per request).",
    ],
    why: [
      "Operating stateful infrastructure correctly is genuinely hard and unforgiving — a botched backup or missed patch can lose or leak data. Most teams shouldn't reinvent that; a managed service buys battle-tested operations and frees your people for the product.",
      "Self-hosting makes sense at very large scale (cost) or for special needs, but for almost everyone the managed option is faster, safer, and cheaper once you count the human time.",
    ],
    breaks: "Assume ‘managed' means ‘I don't have to think' and you'll still get burned: a bad query plan or a missing index is *your* problem, not the provider's. Outgrow your instance with no monitoring and you hit a wall mid-spike. Ignore the bill and convenience gets expensive. And you're now dependent on that provider — migrating a large managed database elsewhere is real work (lock-in). Managed removes the ops toil, not the need to understand your data.",
    scale: "A small app leans entirely on managed services — one managed Postgres, a managed cache, done. As you grow you add read replicas and tune within the service. At very large scale, cost and control sometimes push the biggest players back to self-hosting parts of it with dedicated teams — but that's a deliberate trade made from a position of scale, not a default.",
    related: ["cloud", "cloud-compute", "sharding", "message-queues"],
  },

  "cloud-cost-regions": {
    slug: "cloud-cost-regions",
    title: "Cost, regions & availability zones",
    category: "Cloud",
    color: "blue",
    tagline: "Where your servers live, and why the bill looks the way it does.",
    oneLiner: "Regions and zones decide how close and how resilient your system is; cloud pricing rewards understanding what you actually pay for.",
    what: [
      "Cloud providers split the world into **regions** (geographic areas like ‘Europe-West') and each region into **availability zones** — separate data centres with independent power and network. Putting your system near your users cuts **latency**; spreading it across zones means one data-centre failure doesn't take you down.",
      "Where you run also shapes **cost**. You pay for compute by the second, storage by the gigabyte, and — the sneaky one — **data transfer**, especially data leaving a region (‘egress'). Two architectures that look identical can have very different bills depending on how data moves.",
      "Picking regions and zones, and understanding the pricing model, is part of designing a system — not an afterthought handed to finance.",
    ],
    analogy: {
      title: "Warehouses around the world",
      body: "Imagine running shops worldwide. A region is which country you put a warehouse in — closer to customers means faster delivery. Zones are having two warehouses in that country, so a fire in one doesn't stop shipping. And the courier charges most for parcels crossing borders (egress) — so where you store things, and how often they travel, quietly decides your costs.",
    },
    insideTitle: "The pieces",
    inside: [
      { name: "Region", desc: "A geographic area. Run near users for low latency; near data for compliance." },
      { name: "Availability zone", desc: "An isolated data centre within a region. Span zones to survive one failing." },
      { name: "Egress charges", desc: "Data leaving a region costs money — often the surprise on the bill." },
      { name: "Pricing models", desc: "On-demand, reserved (commit for a discount), and spot (cheap but interruptible)." },
    ],
    how: [
      "You choose a region for each resource and, for resilience, deploy across multiple zones behind a load balancer — if one zone fails, traffic shifts to the others automatically. For global reach you run in several regions and route users to the nearest (often via a CDN or global load balancer).",
      "Cost is managed by right-sizing instances, using reserved or spot capacity for predictable or interruptible work, keeping chatty services in the same region to avoid egress, and watching usage with budgets and alerts so a runaway resource doesn't quietly cost a fortune.",
    ],
    why: [
      "Latency and resilience are user-facing: a system in one distant zone is slow for far-away users and fragile if that zone hiccups. Multi-zone is the cheap insurance every serious system buys. Multi-region is the expensive insurance (and latency win) you buy when you truly need it.",
      "Cost-awareness matters because pay-per-use is double-edged: it scales down beautifully, but without attention it scales *up* your bill just as easily. Understanding what you pay for keeps the trade favourable.",
    ],
    breaks: "Run everything in a single zone and the day that data centre loses power, so does your product. Put your database in one region and your users on another continent, and every request crawls. Shuttle terabytes between regions without noticing and the egress bill dwarfs your compute. Forget to turn off a test cluster and it bills all month. The cloud doesn't protect you from your own architecture — geography and pricing are design decisions.",
    scale: "A side project runs in one region, one zone, on-demand — fine. A real product spans multiple zones for resilience and right-sizes for cost. At global scale you run multiple regions with data-locality and compliance rules, blend reserved/spot capacity, and treat cost as a first-class engineering metric (FinOps) — because at scale a small per-request inefficiency becomes a very large number.",
    related: ["cloud", "cloud-compute", "object-storage-cdn", "observability"],
  },

  "training-vs-inference": {
    slug: "training-vs-inference",
    title: "Training vs inference & cost",
    category: "AI / ML",
    color: "purple",
    tagline: "The two very different jobs in an AI system's life — and which one you pay for forever.",
    oneLiner: "Training is the expensive, one-off process of teaching a model; inference is running it to get answers — cheaper each time, but paid on every single request.",
    what: [
      "An AI model has two phases. **Training** is teaching it: feeding huge datasets and adjusting billions of weights until it's good. It's enormously expensive — clusters of specialised chips running for days or weeks — but it happens relatively rarely.",
      "**Inference** is *using* the trained model: you give it an input, it does one forward pass, you get an answer. Each inference is far cheaper than training — but you do it on **every request**, forever. For a popular product, the lifetime cost of inference dwarfs the one-time cost of training.",
      "This split shapes everything about deploying AI. Training is a research-and-infrastructure problem; inference is a production-engineering and *cost* problem you live with every day.",
    ],
    analogy: {
      title: "Writing the textbook vs answering each student",
      body: "Training is writing the textbook — months of intense, expensive effort, done once. Inference is answering each student's question using that textbook — cheap per question, but done thousands of times a day, every day. Over a year, all those small answers cost far more than writing the book did. AI economics works the same way.",
    },
    insideTitle: "The two phases (and the levers)",
    inside: [
      { name: "Training", desc: "One-off (or occasional) teaching of the model. Huge compute, lots of data, slow." },
      { name: "Inference", desc: "Running the trained model per request. Cheaper each time — but constant." },
      { name: "Fine-tuning", desc: "Cheaply adapting an existing trained model to your task, instead of training from scratch." },
      { name: "Cost levers", desc: "Smaller/quantized models, batching, caching, and hosted API vs self-hosting." },
    ],
    how: [
      "Training runs the full learn-from-data loop (forward pass, measure error, backpropagate, repeat) across massive datasets on GPU/TPU clusters — measured in chip-hours and large bills. Once done, the weights are frozen.",
      "Inference loads those frozen weights and runs only the fast forward pass per request. To tame its relentless cost, teams use smaller or **quantized** models, **batch** many requests together, **cache** repeated answers, and choose between a pay-per-token hosted API and self-hosting on their own GPUs.",
    ],
    why: [
      "Most teams never train a large model from scratch — it's prohibitively expensive and rarely necessary. Instead they **fine-tune** or just call a pre-trained model via an API, turning a research problem into a manageable engineering one.",
      "Understanding the split keeps an AI feature viable: a clever product that costs more per answer than users will pay is a failed product. Inference cost is a core design constraint — part of why grounding with RAG (retrieve, then a cheap focused inference) often beats a giant model answering from memory.",
    ],
    breaks: "Confuse the two and you budget for AI like a one-time purchase, then watch inference costs balloon as usage grows — the bill scales with every request, not once. Pick a giant model when a small one would do and you pay for capability you don't use on every call. Skip caching and batching and you leave money on the table at scale. The model that's cheap to demo can be ruinous to run for a million users.",
    scale: "Prototyping leans on a hosted API — no infrastructure, pay per token. As volume grows, inference cost dominates, so you optimise hard: smaller/quantized models, batching, caching, maybe self-hosting once the math favours it. Training from scratch only makes sense at the largest scale with a real reason; almost everyone fine-tunes or builds on someone else's trained model.",
    related: ["neural-networks", "rag", "embeddings"],
  },

  "secrets-management": {
    slug: "secrets-management",
    title: "Secrets, rotation & least privilege",
    category: "Security",
    color: "pink",
    tagline: "Keeping the keys to the kingdom out of your code — and limiting what each key can do.",
    oneLiner: "Secrets (passwords, API keys, tokens) must live outside your code, be rotated regularly, and grant the least access needed — so a leak is survivable.",
    what: [
      "Every system has **secrets**: database passwords, API keys, signing keys, tokens. The cardinal rule is that they must **never live in your code** or get committed to your repo — because repos get shared, forked, and leaked, and a secret in git history is a secret out forever.",
      "Instead, secrets are supplied from outside — via **environment variables** or a dedicated **secrets manager** (like AWS Secrets Manager or HashiCorp Vault) that stores them encrypted and hands them out only to the things that need them.",
      "Two practices make leaks survivable. **Rotation**: change secrets regularly (and immediately if one leaks), so an old stolen key stops working. **Least privilege**: give each key only the minimum access it needs, so a compromised key can't reach everything.",
    ],
    analogy: {
      title: "Hotel keycards, not a master key",
      body: "A hotel doesn't give every guest a master key to all rooms. Each gets a keycard that opens only their room, expires at checkout, and can be cancelled instantly if lost. Secrets should work the same way — scoped to the minimum (least privilege), expiring and replaceable (rotation) — never one all-powerful master key copied everywhere.",
    },
    insideTitle: "The practices",
    inside: [
      { name: "Secrets manager", desc: "An encrypted vault that stores secrets and serves them to authorised services only." },
      { name: "Environment variables", desc: "Secrets injected at runtime from outside the code — never hard-coded." },
      { name: "Rotation", desc: "Regularly replacing secrets, and immediately revoking any that leak." },
      { name: "Least privilege", desc: "Each key/role gets the minimum access it needs — shrinking the blast radius of a leak." },
    ],
    how: [
      "At deploy time the platform injects secrets into the app's environment (or the app fetches them from a secrets manager at startup), so the running process has them but the codebase never does. Access to the vault is itself controlled — only specific services or roles can read specific secrets.",
      "Rotation is automated where possible: the manager issues short-lived credentials that expire on their own, so there's nothing long-lived to steal. Least privilege is enforced by giving each service its own narrowly-scoped key rather than a shared, all-powerful one.",
    ],
    why: [
      "Secrets are the highest-value target — one leaked database password can expose every customer. Keeping them out of code removes the most common leak path (the repo). Rotation limits how long a stolen secret is useful. Least privilege limits how much damage any single leak can do. Together they turn ‘catastrophe' into ‘contained incident'.",
      "The alternative — hard-coded, long-lived, broadly-scoped secrets — means one mistake (a public repo, a logged key) hands an attacker the whole system, indefinitely.",
    ],
    breaks: "Commit an API key to a public repo and bots will find and abuse it within minutes — it happens constantly. Use one all-powerful database credential everywhere and a single leak exposes everything. Never rotate, and a key stolen years ago still works today. Log a secret to your monitoring system and you've leaked it to everyone with log access. The damage from secrets done wrong is total and fast; the discipline to do them right is cheap.",
    scale: "A small app can use platform environment variables and rotate by hand. As you grow, a real secrets manager with access controls and automated, short-lived credentials becomes essential. At enterprise scale: per-service identities, automatic rotation, audit logs of every secret access, and least-privilege enforced everywhere — because the blast radius of one mistake grows with the system.",
    related: ["auth", "web-security", "observability"],
  },
  ...
  "threat-modelling": {
    ...
    category: "Security",
    color: "pink",
    tagline: "Thinking like an attacker on purpose — before they do it for real.",
    oneLiner: "Threat modelling is the practice of systematically asking 'how could this be attacked?' so you find and fix weaknesses by design, not after a breach.",
    what: [
      "Most security work is reactive — patch the hole after someone finds it. **Threat modelling** is proactive: you sit down, look at your system, and deliberately ask *how would someone attack this?* before shipping. It's a structured imagination exercise that turns ‘we hope it's secure' into ‘we've thought about how it breaks'.",
      "It revolves around a few questions: **what are we building** (a diagram), **what can go wrong** (the threats), **what will we do about it** (mitigations), and **did we do a good job** (review). The point is to find weaknesses on a whiteboard, where they're cheap to fix, instead of in production.",
      "A common lens is **STRIDE** — checking each part of the system for Spoofing, Tampering, Repudiation, Information disclosure, Denial of service, and Elevation of privilege. It's a checklist so you don't forget a whole category of attack.",
    ],
    analogy: {
      title: "A burglar walking your house with you",
      body: "Before you move in, you walk through the house with an ex-burglar who points at every weak spot: ‘that window's hidden from the street, this lock is flimsy, the side gate has no light'. You fix them now, cheaply, instead of after a break-in. Threat modelling is that walk-through for your software — deliberately seeing it through an attacker's eyes.",
    },
    insideTitle: "The pieces",
    inside: [
      { name: "Data-flow diagram", desc: "A map of the system — components, data, and the trust boundaries between them." },
      { name: "Trust boundaries", desc: "The lines where data crosses from less-trusted to more-trusted — the danger zones." },
      { name: "STRIDE (or similar)", desc: "A checklist of attack categories so no whole class of threat is forgotten." },
      { name: "Mitigations & risk", desc: "For each plausible threat: how likely, how bad, and what we'll do about it." },
    ],
    how: [
      "You draw the system as a **data-flow diagram**, marking **trust boundaries** — the points where untrusted input meets trusted logic (a user hitting your API, a service calling another). Those boundaries are where threats concentrate.",
      "Then, boundary by boundary, you walk a checklist like STRIDE and ask ‘could someone spoof an identity here? tamper with this data? read what they shouldn't?'. Each real threat gets a **mitigation** (or an accepted risk), and the whole thing is revisited as the system changes.",
    ],
    why: [
      "Finding a design flaw on a whiteboard costs a meeting; finding it via a breach costs customers, data, and trust. Threat modelling shifts security *left* — earlier, where it's cheapest — and makes it systematic instead of relying on someone happening to think of an attack.",
      "The alternative — security as an afterthought, or a single penetration test right before launch — catches some things but misses design-level flaws that should never have been built.",
    ],
    breaks: "Skip it and you build whole categories of weakness in by design — an endpoint with no authorization check, a trust boundary you never noticed, a denial-of-service waiting to happen — and only discover them when an attacker does. Do it once and never revisit, and it goes stale as the system grows new attack surface. Treat it as box-ticking rather than genuine adversarial thinking and it finds nothing. The value is in honestly asking ‘how would I break this?'.",
    scale: "A small team can threat-model informally — a whiteboard and the STRIDE questions over coffee. As the system grows, it becomes a standard step in designing any significant feature, with diagrams kept up to date. At enterprise scale it's a formal, recurring discipline tied to compliance, with dedicated security review of every major change — because the cost of a missed threat scales with the number of users behind it.",
    related: ["web-security", "auth", "secrets-management"],
  },
};

// Domain content lives in lib/content/*.js and is merged here, so the whole app
// (routing, sidebar, repository) sees one TECH_CONTENT. The build scripts
// auto-discover the same files via vm; the typeof guards keep that path safe.
export const TECH_CONTENT = {
  ...BASE_CONTENT,
  ...(typeof CS_CORE !== "undefined" ? CS_CORE : {}),
  ...(typeof SYSTEMS_NETWORKS !== "undefined" ? SYSTEMS_NETWORKS : {}),
  ...(typeof CRAFT_SD !== "undefined" ? CRAFT_SD : {}),
  ...(typeof WEB_DEEP !== "undefined" ? WEB_DEEP : {}),
  ...(typeof BACKEND_DEEP !== "undefined" ? BACKEND_DEEP : {}),
  ...(typeof AI_SEC_DEEP !== "undefined" ? AI_SEC_DEEP : {}),
  ...(typeof MOBILE_DEVOPS_DEEP !== "undefined" ? MOBILE_DEVOPS_DEEP : {}),
  ...(typeof CLOUD_DATAENG_DEEP !== "undefined" ? CLOUD_DATAENG_DEEP : {}),
  ...(typeof FOUNDATIONS_DEEP !== "undefined" ? FOUNDATIONS_DEEP : {}),
  ...(typeof DATA_DEEP !== "undefined" ? DATA_DEEP : {}),
  ...(typeof WEB_PLATFORM_DEEP !== "undefined" ? WEB_PLATFORM_DEEP : {}),
  ...(typeof SYSTEMS_OPS_DEEP !== "undefined" ? SYSTEMS_OPS_DEEP : {}),
};
